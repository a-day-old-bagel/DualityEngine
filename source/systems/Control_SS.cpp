...
namespace DualityEngine {
    ...
    bool System_Control_SS::onInit(std::stringstream& output) {
        return true;
    }

    void System_Control_SS::onClean() {

    }

    void System_Control_SS::onTick() {
        ...
        if (bank->pSpaceControlCurrent->throttle[ControlSS::LINBRAKE]) { // apply braking vector to throttle values...
            ...
            if (vectorVel.x || vectorVel.y || vectorVel.z) {
                // if velocity is too small the braking algorithm produces floating point errors, so a deadzone was
                // introduced. Will zero velocity if already below a threshold magnitude.
                if (glm::length(vectorVel) < DUA_CONTROLSS_LINEAR_BRAKING_DEADZONE) {
                    ...
                    // scale to full throttle on the control axis most aligned with motion,
                    // scale other axes by the same value.
                    ...
                    // add the appropriate values to the control component's throttle array
                    // (allowing no value to pass 1)
                    for (uint i = 0; i < 3; ++i) {
                        ...
                        if (!bank->pSpaceControlCurrent->throttle[i * 2 + int(axisDotProds[i] <= 0)]) {
                            ...
                            bank->pSpaceControlCurrent->throttle[thrustMostOppositeVelocity] +=
                                    fabs(axisDotProds[i]) * breakingVectorComponentMultiplier
                                    * (std::min((axisDotProds[i] * (-1 + 2 * opposingThrustDecider)) /
                                                bank->pSpaceControlCurrent->thrust[thrustMostOppositeVelocity], 1.f))
        ...
        if (bank->pSpaceControlCurrent->throttle[ControlSS::ANGBRAKE] &&
            (bank->pSpaceControlCurrent->angBrakeEngageCounter++ >
             DUA_CONTROLSS_ANGULAR_BRAKING_ENGAGE_COUNT)) { // apply braking vector to throttle values...
            ...
            if (vectorVel.x || vectorVel.y || vectorVel.z) {
                ...
                if (glm::length(vectorVel) < DUA_CONTROLSS_ANGULAR_BRAKING_DEADZONE) {
                    ...
                    // find which angveloc component is largest in order to scale that torques's throttle to full
                    // (scaling others by same factor, resulting in less-than-full throttles)
                    ...
                    for (uint i = 0; i < 3; ++i) {
                        ...
                        if (!bank->pSpaceControlCurrent->throttle[i * 2 + 6 + int(vectorVel[i] <= 0)]) {
                            ...
                            bank->pSpaceControlCurrent->throttle[thrustMostOppositeVelocity] +=
                                    fabs(vectorVel[i]) * breakingVectorComponentMultiplier
                                    * (std::min((vectorVel[i] * (-1 + 2 * opposingThrustDecider)) /
                                                bank->pSpaceControlCurrent->thrust[thrustMostOppositeVelocity], 1.f))
        ...
        bank->getLinearVelocPtr(bank->activeControlID)->applyImpulse(
                bank->pSpaceControlCurrent->currentAxes[0] *
                (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::FORWARD], 1.f) *
                 bank->pSpaceControlCurrent->thrust[ControlSS::FORWARD])
                - bank->pSpaceControlCurrent->currentAxes[0] *
                  (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::BACKWARD], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::BACKWARD])
                + bank->pSpaceControlCurrent->currentAxes[1] *
                  (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::RIGHT], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::RIGHT])
                - bank->pSpaceControlCurrent->currentAxes[1] *
                  (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::LEFT], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::LEFT])
                + bank->pSpaceControlCurrent->currentAxes[2] *
                  (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::UP], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::UP])
                - bank->pSpaceControlCurrent->currentAxes[2] *
                  (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::DOWN], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::DOWN])
        );

        bank->getAngularVelocPtr(bank->activeControlID)->applyImpulse(
                (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::PITCHPOS], 1.f) *
                 bank->pSpaceControlCurrent->thrust[ControlSS::PITCHPOS])
                - (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::PITCHNEG], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::PITCHNEG]),
                (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::YAWPOS], 1.f) *
                 bank->pSpaceControlCurrent->thrust[ControlSS::YAWPOS])
                - (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::YAWNEG], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::YAWNEG]),
                (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::ROLLPOS], 1.f) *
                 bank->pSpaceControlCurrent->thrust[ControlSS::ROLLPOS])
                - (std::min(bank->pSpaceControlCurrent->throttle[ControlSS::ROLLNEG], 1.f) *
                   bank->pSpaceControlCurrent->thrust[ControlSS::ROLLNEG])
        );
