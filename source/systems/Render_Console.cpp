...
//        DUA_uint16 indices[sizeIndexArray];
        ...
        float halfScreenX = (float) Settings::Display::screenResX * 0.5f;
        float halfScreenY = (float) Settings::Display::screenResY * 0.5f;
        ...
        for (int i = 0; i < numCharsY_body; i++) {
            for (int j = 0; j < numCharsX; j++) {
                ...
                verts[STRIDE(8) + 0] = ((marginWidth + j * charStepX) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 1] = ((marginHeight + i * charStepY) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 2] = ((marginWidth + j * charStepX + charWidth) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 3] = ((marginHeight + i * charStepY) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 4] = ((marginWidth + j * charStepX) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 5] = ((marginHeight + i * charStepY + charHeight) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 6] = ((marginWidth + j * charStepX + charWidth) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 7] = ((marginHeight + i * charStepY + charHeight) + screenOffsetY) / halfScreenY + 1;
        ...
        for (int i = numCharsY_body; i < numCharsY_body + numCharsY_comm; i++) {
            for (int j = 0; j < numCharsX; j++) {
                // Vertices of each character quad - four per quad
                verts[STRIDE(8) + 0] = ((marginWidth + j * charStepX) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 1] = ((2 * marginHeight + i * charStepY) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 2] = ((marginWidth + j * charStepX + charWidth) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 3] = ((2 * marginHeight + i * charStepY) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 4] = ((marginWidth + j * charStepX) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 5] = ((2 * marginHeight + i * charStepY + charHeight) + screenOffsetY) / halfScreenY + 1;

                verts[STRIDE(8) + 6] = ((marginWidth + j * charStepX + charWidth) + screenOffsetX) / halfScreenX - 1;
                verts[STRIDE(8) + 7] = ((2 * marginHeight + i * charStepY + charHeight) + screenOffsetY) / halfScreenY + 1;
